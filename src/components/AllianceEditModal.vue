...
export default {
  ...
  props: ['type', 'title', 'submit', 'prevParams'],
  emits: ['submit-function', 'change-icon'],
  data: function () {
    return {
      images: [],
      newAllianceParams: { name: this.prevParams.name, icon: this.prevParams.icon, description: this.prevParams.description },
    };
  },
  mounted: function () {
  ...
  methods: {
    ...
    submitAllianceParams: function (params) {
      this.$emit('submit-function', params)
    }
  }
}
</script>
...
<template>
  <div class="modal fade" :id="type" tabindex="-1" aria-labelledby="submitModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <form v-on:submit.prevent="submitAllianceParams(newAllianceParams)">
